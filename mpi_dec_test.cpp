...
//                AVFrame ffm_frame;
//                uint8_t *base = (uint8_t *)mpp_buffer_get_ptr(buff);
//                ffm_frame.data[0]=base;
//                ffm_frame.height = mpp_frame_get_height(frame);
//                ffm_frame.width = mpp_frame_get_width(frame);
//                ffm_frame.format = (enum AVPixelFormat)fmt;
//                avframe_to_cvmat(&ffm_frame);
                ...
                else
                {
                    ...
                    if (base != NULL)
                    {
                        ...
                        std::string fileName="img/"+std::to_string(data->frame_count)+".jpg";
...
int mpi_dec_test_decode(MpiDecTestCmd *cmd)
{
    ...
    // decoder demo
    //创建 MPP context 和 MPP api 接口。 （注意，和RGA一样，多个线程多个实例需要多个独立的的context）
    ...
    //设置一些MPP的模式（这里设置的是 MPP_DEC_SET_PARSER_SPLIT_MODE）
...
int main(int argc, char **argv)
{
    ...
    char * filePath = "/media/linaro/A20E-4639/output.h264";
    //char * filePath = "/home/linaro/1.h264";
    memcpy(cmd->file_input,filePath,strlen(filePath));
    cmd->have_input=1;
    cmd->type = (MppCodingType)7;
    ret =0;
    if (ret) {
        if (ret < 0) {
